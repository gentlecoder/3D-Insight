/**
 * Created by gentlecoder on 2017/8/11.
 */
$(function() {
  ...
  var line,
    lineHeight,
    lineAnimateBeginInterval,
    linePhysicPositionYBefore,
    linePoolPositionYBefore,
    linePhysicPositionYAfter,
    linePoolPositionYAfter;
  ...
  function init() {
    ...
    generatePhysicBoxCondition(
      data,
      -400 - 112,
      50 * (parseInt(physicBoxNumSZ / xNum / yNum) + 1),
      physicBoxNumSZ,
      0
    );
    generatePhysicBoxCondition(
      data,
      0 - 112,
      50 * (parseInt(physicBoxNumNJ / xNum / yNum) + 1),
      physicBoxNumNJ,
      1000
    );
    generatePhysicBoxCondition(
      data,
      400 - 112,
      50 * (parseInt(physicBoxNumXA / xNum / yNum) + 1),
      physicBoxNumXA,
      2000
    );
  ...
  function generatePlane(
    ...
    planeGeometryPhysic[0] = new THREE.PlaneGeometry(
      100 * (parseInt(physicBoxNumSZ / xNum / yNum) + 1),
      300
    );
    planeGeometryPhysic[1] = new THREE.PlaneGeometry(
      100 * (parseInt(physicBoxNumNJ / xNum / yNum) + 1),
      300
    );
    planeGeometryPhysic[2] = new THREE.PlaneGeometry(
      100 * (parseInt(physicBoxNumXA / xNum / yNum) + 1),
      300
    );
    ...
    for (var i = 0; i < 3; i++) {
      ...
      new TWEEN.Tween(planePool[i].position)
        ...
        .start();
      planePhysic[i] = new THREE.Mesh(
        planeGeometryPhysic[i],
        planeMaterialPhysic
      );
  ...
  function generatePhysicBox(xNum, yNum, physicBoxNum, xPosition) {
    ...
    for (var i = 0; i < zNumLast % yNum; i++) {
      physicBox[i] = new THREE.Mesh(cubeGeometryPhysic, cubeMaterialPhysic);
      physicBox[i].position.set(
        xPosition + 100 * parseInt(zNumLast / yNum),
        40 + 20 * i,
        (100 * zNum) / 2 - 100 * zNum
      );
      physicBox[i].castShadow = true;
  ...
  function generatePhysicBoxCondition(
    data,
    positionX,
    positionZ,
    totalNum,
  ...
  function generatePoolBox() {
    ...
    var type = ["CPU", "内存", "存储"];
    for (var i = 0; i < 3; i++) {
      cubeTrans[i] = new THREE.Mesh(
        cubeGeometryPoolTrans,
        cubeMaterialPoolTrans
      );
  ...
  function showUsageRate(rate, positionX, type) {
    ...
    texture.needsUpdate = true;
    var mesh = new THREE.Mesh(
      new THREE.PlaneGeometry(50, rate),
      new THREE.MeshBasicMaterial({
        transparent: true,
        map: texture,
        side: THREE.DoubleSide
  ...
  function render() {
    ...
    for (var i = 0; i < 6; i++) {
      if (
        (line[i].geometry.vertices[0].y > linePhysicPositionYAfter && i == 0) ||
        (line[i].geometry.vertices[0].y > linePhysicPositionYAfter &&
          line[i - 1].geometry.vertices[0].y <
            linePhysicPositionYBefore - lineAnimateBeginInterval &&
          i > 0)
      ) {
        ...
        (i == 3 &&
          line[i].geometry.vertices[0].y > linePoolPositionYAfter &&
          line[i - 1].geometry.vertices[0].y <
            linePhysicPositionYBefore - lineAnimateBeginInterval) ||
        (i > 3 &&
          line[i].geometry.vertices[0].y > linePoolPositionYAfter &&
          line[i - 1].geometry.vertices[0].y <
            linePoolPositionYBefore - lineAnimateBeginInterval)
      ) {
        line[i].geometry.vertices[0].y -= 5;
        line[i].geometry.verticesNeedUpdate = true;
        line[i].geometry.lineDistancesNeedUpdate = true;
